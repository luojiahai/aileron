...
export function isBotReviewer(comment: T.ReviewComment | null): boolean {
  if (comment === null) return false;
  const author = (comment.authorLogin ?? "").toLowerCase();
  return author.endsWith("[bot]") || BOT_REVIEWER_LOGINS.has(author);
}
...
export function parseReviewThreads(value: unknown): readonly T.ReviewThread[] {
  ...
  for (const thread of threads) {
    if (!isBotReviewer(thread.firstComment)) continue;
  ...
  return threads
    ...
    .map(({ id, firstComment }) => ({
      id,
      firstComment,
      isBotReviewer: isBotReviewer(firstComment),
      botReviewPasses: passes,
    }));
}
