...
it("classifies bot reviewers by login only", () => {
  const comment = (login: string, body = "") => ({
    authorLogin: login,
    body,
    path: null,
    line: null,
  ...
  expect(isBotReviewer(comment("claude"))).toBe(true);
  expect(isBotReviewer(comment("coderabbitai"))).toBe(true);
  expect(isBotReviewer(comment("copilot-pull-request-reviewer"))).toBe(true);
  expect(isBotReviewer(comment("Some-App[bot]"))).toBe(true);
  expect(isBotReviewer(comment("alice", "RUN_ID: run-1 severity high"))).toBe(false);
  expect(isBotReviewer(null)).toBe(false);
});

it("annotates bot-reviewer threads with distinct review-pass counts", () => {
  const response = {
    data: {
      repository: {
        pullRequest: {
                  ...
                  nodes: [
                    {
                      ...
                      path: null,
                      line: null,
                      author: { login: "some-review-app[bot]" },
                    },
                  ],
                },
              },
  ...
  expect(threads.map((thread) => thread.isBotReviewer)).toEqual([true, true]);
  expect(threads.map((thread) => thread.botReviewPasses)).toEqual([3, 3]);
});
