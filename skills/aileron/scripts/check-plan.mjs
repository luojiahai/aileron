...
const LANES = /Ten lanes on (the bulk model|`[a-z]+`) at the PR head/;
const SUB_BLOCKS = [
	"Depends on.",
	"Files.",
	"Build.",
...
const PROGRAM_MARKERS = ["/loop", "git show origin/main:", "status message"];
...
for (const pr of prSections) {
	...
	if (live) {
		if (!LANES.test(live.rest)) fail(live.n, `${pr.title}: Verify, live lacks "Ten lanes on the bulk model at the PR head"`);
