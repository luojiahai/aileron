...
**Titles.** Use Conventional Commits in the form `type(scope): subject`. Use `feat`, `fix`, `docs`, `refactor`, `test`, `chore`, or `perf` as the type. Use the changed area, such as `aileron` or `scripts`, as the scope. Keep the subject short and imperative. Apply the same `/technical-writing` and `/unslop` pass as the body. Name a real symbol when one carries the change. For example, `fix(aileron): retarget opening-a-pr babysit trigger`. Do not add a trailing period.
...
- `## Verification`. State how you ran each check and its rigor. Name the real path, such as the project's `verify-<app>` skill or the targeted tests. State the outcome of each check, not only the command name.
