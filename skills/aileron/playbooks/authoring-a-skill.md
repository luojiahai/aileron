...
1. Write the SKILL.md directly. Frontmatter carries `name` (kebab-case, matching the directory), `description`, and `disable-model-invocation` when the skill is user-invoked only. A personal skill lands in `~/.claude/skills/<name>/SKILL.md`, a project skill in `.claude/skills/<name>/SKILL.md`, a plugin skill in the plugin's `skills/<name>/SKILL.md`.
