...
Use this reference when the Babysit playbook (`../playbooks/babysit.md`) handles bot-reviewer comments (Claude code review, CodeRabbit, an agentic security review, any GitHub App login ending in `[bot]`). The goal is not to ignore the bot by default. The goal is to stop treating every comment as a required code change.

## Decision rubric

Classify each bot-reviewer thread before acting:
...
- Confidence: candidate
- Skip when: the bot flags an export, component, helper, or file as unused, and `gt ls -s`, upper-stack diffs, or PR context shows it is used by a later PR in the stack.
...
- Source: one sticky-occlusion PR: six bot review passes, roughly eighteen findings, every one fixed rather than dismissed.
...
- Source: one prose-pinning PR with eight bot review passes; the claim was real on
