...
At the end of the run, before handing back, check the log told the truth. Read this run's transcript under `~/.claude/projects/<slug>/` (slug is the absolute working directory with `/` and `.` replaced by `-`). Don't glob across `~/.claude/projects/*/`; that reads unrelated private chats. Walk the log against what actually happened:
...
Before handing back, you must spawn a fresh subagent to review the trail, on a different model tier from the one that did the work when the models file allows it. Self-review is not a substitute; the point is fresh eyes and a clean context you cannot bring yourself. The subagent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.
