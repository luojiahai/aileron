...
Before spawning investigators, list the available MCP tools. Use the tools listed in the session when present. Otherwise run `ToolSearch` over likely names (linear, jira, notion, confluence, slack, datadog, sentry, databricks, snowflake) to discover deferred MCP tools.
...
- `model`: your configured `why investigators` model from `~/.claude/aileron-models.md` (default `sonnet`). A value of `inherit` means omit `model`.
- `run_in_background`: `true`

Investigators shouldn't write anything. That's a posture, not a sandbox; the prompt says so.
...
The synthesizer's quality check spot-verifies citations, which can require MCP access, so it gets the same tool access as the investigators.
...
After the Sources Consulted block, if the user's `why` question leads into actually changing this code, convert the lineage findings into a Preserve / Change / Avoid / Risk constraint set suitable for planning the change.
