...
2. **Critique.** Explain first, then spawn several independent critics to identify architectural issues
...
- `model`: your configured `how explorer` model from `~/.claude/aileron-models.md` (default `sonnet`)
...
After the explanation is complete, spawn one architectural critic per entry in your configured `how critics` list (defaults `fable`, `opus`, `sonnet`), all in a single message. Give each critic a distinct primary lens from the rubric (boundaries and ownership, data flow and state, failure modes and coupling) so they start from different questions; every critic still covers the whole rubric.

For each critic:
- `subagent_type`: `Explore`
- `model`: one entry from the configured `how critics` list. A value of `inherit` means omit `model`. The lead should escalate a critic to `fable` when the architecture warrants deeper analysis.
- `run_in_background`: `true`
