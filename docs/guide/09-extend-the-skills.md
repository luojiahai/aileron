...
```text
/aileron write a skill for verifying database migrations in this repo
```

Writing a skill matches the [Authoring or modifying a skill playbook](../../skills/aileron/playbooks/authoring-a-skill.md), which writes the `SKILL.md` directly to `~/.claude/skills/<name>/` for a personal skill or `.claude/skills/<name>/` for a project one, validates the frontmatter and links, and ships the result through the Opening a PR playbook. Agent-facing prose has a higher bar than human prose, because an unhelpful sentence becomes an instruction some future agent follows. Let the playbook hold that bar rather than writing a `SKILL.md` freehand.
