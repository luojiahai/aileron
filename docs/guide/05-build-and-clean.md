...
```text
/aileron this command emits two records after a retry. repro first, then fix and verify.
```

A feature prompt states the behavior and what must not change:

```text
/aileron add a --json flag. text output stays byte-identical. verify both forms.
```

A refactoring prompt pins behavior before structure moves:

```text
/aileron move parsing into one module, zero behavior change. record the current output first and prove it's unchanged after.
```

A perf prompt states the measurement, not a vibe:

```text
/aileron startup takes 1.8s on this fixture. trace it, fix the measured cause, show me before and after.
```

Each of these routes to its playbook ([Bug fix](../../skills/aileron/playbooks/bug-fix.md), [Feature](../../skills/aileron/playbooks/feature.md), [Refactoring](../../skills/aileron/playbooks/refactoring.md), [Perf issue](../../skills/aileron/playbooks/perf-issue.md)), and the playbook supplies the steps you didn't type: reproduce before fixing, name the data shape before implementing, pin behavior before restructuring, profile before optimizing.

For sustained improvement of one number, there's the [Hillclimb playbook](../../skills/aileron/playbooks/hillclimb.md). Give it the metric, a target, and a floor on attempts, and it loops one hypothesis at a time with a frozen measurement harness. It keeps wins and reverts everything else.
...
The [Opening a PR playbook](../../skills/aileron/playbooks/opening-a-pr.md) runs [`/unslop`](../../skills/unslop/SKILL.md) on the diff's prose and [`/no-comments`](../../skills/no-comments/SKILL.md) on its comments before each commit, and applies `/unslop` to the PR description and commit bodies. If the diff still feels padded, ask for the rest in plain words: remove unsupported guards, dead compatibility paths, and unrelated edits.
...
The division of labor is worth keeping straight. `/unslop` cleans slop out of prose, in the diff and around it, and `/no-comments` hands the comments to a reviewer who didn't write them.

**Pitfall:** cleanup is not optional polish. A diff with narrating comments and defensive dead weight reads as unfinished to reviewers, and the extra code is where the next bug hides. If the diff feels padded, say `unslop it, then no-comments` before you commit, not after review calls it out.
