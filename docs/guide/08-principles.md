...
aileron ships 21 principles as individual skills. `/aileron` reads their index at the start of every multi-step task, applies the ones the task triggers, and names each applied principle in its reply along with the decision it changed.
...
Next: [Recipes and pitfalls](./09-recipes-and-pitfalls.md).
